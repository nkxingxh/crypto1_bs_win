...
void notify_status_offline(int sig)
{
    ...
    printf(/*VT100_cleareol */ "计算中... %6.02f%%  速度: %" llu " keys/s  预计需要时间: %" llu " s\n", (100.0 * total_states_tested / (total_states)), now_v, now_need_time);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        printf("用法: %s <nonces.txt> <uid> [线程数]\n", argv[0]);
        return -1;
    }
...
#endif
*/
    if (argc >= 4) thread_count = *argv[3] - '0';
    else thread_count = 4;
